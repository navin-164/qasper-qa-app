...
def extract_sections_and_paragraphs(paper: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for sec_idx, paragraphs in enumerate(section_paragraphs):
        ...
        clean_section_name = normalize_text(section_name)

        for para_idx, para in enumerate(paragraphs):
            raw_text = normalize_text(para)
            if not raw_text:
                continue

            # --- CONTEXTUALIZED CHUNKING ---
            # Injecting title and section into the text so vector embeddings capture global context
            contextualized_text = f"Paper Title: {paper_title}. Section: {clean_section_name}. {raw_text}"
            # -------------------------------
            ...
            records.append(
                {
                    ...
                    "text": contextualized_text,  # Save the enriched text
                }
            )
    return records
...
def main() -> None:
    ...
    parser.add_argument("--max_papers", type=int, default=10) # Default to 10 for a lightweight MVP
    args = parser.parse_args()

    ensure_dirs(BACKEND_ROOT)
    ...
    write_jsonl(BACKEND_ROOT / "data/raw/papers.jsonl", paper_rows)
    write_jsonl(BACKEND_ROOT / "data/processed/paragraphs.jsonl", paragraph_rows)
    write_jsonl(BACKEND_ROOT / "data/raw/qasper_eval_qas.jsonl", eval_rows)
